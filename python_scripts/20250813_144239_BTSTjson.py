...
def calculate_btst_risk_management(latest, prev_day, atr, side: str = 'LONG'):
    """Directional BTST risk management.

    For LONG: stop_loss below price, target above.
    For SHORT: stop_loss above price, target below (stop > target enforced).
    """
    side = side.upper()
    ...
    if side == 'SHORT':
        ...
        if stop_loss <= target:  # enforce ordering
            gap = round(latest['Close'] * 0.005, 2)
            stop_loss = round(latest['Close'] + gap, 2)
            target = round(latest['Close'] - gap, 2)
    ...
    if intraday_range > 0:
        ...
        close_position = (latest['Close'] - latest['Low']) / intraday_range if intraday_range else 0
    ...
    if target <= stop_loss:  # safety
        gap = round(latest['Close'] * 0.005, 2)
        target = round(latest['Close'] + max(gap, 0.5), 2)
    return stop_loss, target
...
def analyze_stock(symbol):
    try:
        ...
        if latest['Open'] == latest['High']:
            ...
            stop_loss, target = calculate_btst_risk_management(latest, prev_day, atr, side='SHORT')
            condition = "Open=High (Bearish)"
            models_used['Open-High'] = {'signal': 'Bearish', 'confidence': 30}
        elif latest['Open'] == latest['Low']:
            primary_signal = "Buy"
            confidence = max(confidence, 70)
            stop_loss, target = calculate_btst_risk_management(latest, prev_day, atr, side='LONG')
        ...
        if stop_loss is None and recommendation != "Neutral":
            if recommendation in ("Buy", "BTST Buy"):
                side = 'LONG'
            else:
                side = 'SHORT'
            stop_loss, target = calculate_btst_risk_management(latest, prev_day, atr, side=side)
        ...
        if btst_metrics:
            result['BTST Metrics'] = btst_metrics
...
def main():
    ...
    if not results:
        ...
        return json.dumps({"error": "No valid data could be fetched"}, indent=2)
    ...
    analysis_results = {
        "metadata": {
            ...
            "average_confidence": round(sum(r['Confidence (%)'] for r in results) / len(results), 1),
    ...
    actionable_recommendations = [
        r for r in results 
        if r['Recommendation'] in ['Buy', 'Sell', 'BTST Buy'] 
        and r['Confidence (%)'] >= min_confidence
    ]
    
    # Filter BTST opportunities
    btst_opportunities = [
        r for r in actionable_recommendations 
        if 'BTST Metrics' in r 
        and r['BTST Metrics']['BTST Score'] >= btst_threshold 
        and r['Recommendation'] == 'BTST Buy'
    ]
    
    # Update metadata counts
    analysis_results["metadata"]["actionable_recommendations"] = len(actionable_recommendations)
    analysis_results["metadata"]["btst_opportunities"] = len(btst_opportunities)
    
    # Add actionable recommendations and BTST opportunities
    analysis_results["actionable_recommendations"] = sorted(
        actionable_recommendations, 
        key=lambda x: x['Confidence (%)'], 
        reverse=True
    )
    
    analysis_results["btst_opportunities"] = sorted(
        btst_opportunities, 
        key=lambda x: x['BTST Metrics']['BTST Score'], 
        reverse=True
    )
    
    # Calculate risk-reward for BTST opportunities
    for opp in analysis_results["btst_opportunities"]:
        current_price = opp['Current Price']
        stop_loss = opp['Risk Management']['Stop Loss']
        target = opp['Risk Management']['Target']
        
        risk = current_price - stop_loss
        reward = target - current_price
        opp['Risk_Reward_Ratio'] = round(reward / risk, 2) if risk > 0 else 0
    ...
    print("\nAnalysis Summary:")
    print(f"Total stocks analyzed: {analysis_results['metadata']['total_stocks_analyzed']}")
    print(f"Actionable recommendations: {analysis_results['metadata']['actionable_recommendations']}")
    print(f"BTST opportunities: {analysis_results['metadata']['btst_opportunities']}")
    print(f"Average confidence: {analysis_results['metadata']['average_confidence']}%")
